...
- prompt: |
    ...
    - 완료 보고 형식에 맞춰 보고
    - 사용자 승인 후에만 main 병합
    - 병합 후 worktree 정리: `git worktree remove ../math-report-phase1-auth`
```
...
### 사용자: "T2.1 시험지 분석 API 구현해줘"

```
## 작업 분석

요청: 시험지 분석 API 구현
태스크: Phase 2, T2.1: 시험지 분석 API
...
## 의존성 확인
- 선행 태스크: T1.x (인증) - 완료됨
- 병렬 가능: T2.2 (UI)와 병렬 가능 (Mock 사용)

## 실행

backend-specialist 에이전트를 호출합니다.
