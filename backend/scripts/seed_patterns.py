...
- 도형의 성질 → 도형""",
    ...
    # ==========================================
    {
        "name": "난이도 상세 판정 기준",
        "template_type": "analysis_guide",
        "content": """## 난이도 판정 상세 기준 (극도로 보수적)

🚨 **원칙: 확신이 없으면 무조건 한 단계 낮게! 상은 킬러 문제만!**

### 상(high) - "킬러 문제" 수준만 (극히 제한적)
**반드시 다음 조건을 모두 충족:**
1. 아래 특징 중 **4개 이상** 명확하게 해당
2. 시험에서 **가장 어려운 1문제** (예외적으로 2문제)
3. 배점 10점 이상 서술형 (단, 배점만으로 상 판단 금지!)
4. **수능/모의고사 킬러 문제 수준**

**특징:**
- 3개 이상의 서로 다른 대단원 개념을 복합 적용
- 6단계 이상의 매우 복잡한 풀이 과정
- 복잡한 조건 분기(3가지 이상 경우의 수)
- 논리적 증명 요구 (계산만으로 불가능)
- 교과서에 없는 창의적/통찰력 필요
- 학생 정답률 30% 이하 예상
...
- ❌ 계산이 복잡한 것 (개념은 단순)
- ❌ 단순 서술형 (배점 높아도 중)
- ❌ 일반적인 고난도 문제 (매우 어려워야 상)

**예시:**
- "연립방정식의 정수해 구하기 (8점)" → 중
- "이차함수 최댓값 구하기 (10점)" → 중
- "두 조건 동시 만족하는 n (창의적, 3개 대단원)" → 상 후보

**원칙:**
- 시험당 상은 **최대 1문제** (20문제 기준, 5%)
- 25문제 이상일 때만 예외적으로 2문제 (8%)
- **배점 높다고 자동으로 상 아님!**

### 중(medium) - 기본값 (애매하면 무조건 여기)
- 교과서 응용/변형 문제
- 2-4단계 풀이
- 공식 변형 적용
- 2개 개념 연결
