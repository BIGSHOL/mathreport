...
3. **학습 처방**
   ...
   - 복습 전략 제안""",
    ...
    # ==========================================
    {
        "name": "난이도 상세 판정 기준",
        "template_type": "analysis_guide",
        "content": """## 난이도 판정 상세 기준 (보수적 적용)

⚠️ **난이도는 보수적으로 판정합니다. 애매한 경우 낮은 난이도를 선택하세요.**

### 상(high) - 다음 중 3개 이상 해당하고, 시험에서 최고 난이도 문항일 때만
- 3개 이상의 서로 다른 개념을 복합적으로 적용
- 5단계 이상의 복잡한 풀이 과정
- 조건 분기(경우 나누기) 필요
- 증명이나 논리적 서술 요구 (단순 계산이 아닌)
- 전형적이지 않은 창의적 접근 필요
- 일반적으로 8점 이상의 고배점 서술형

**주의**: 상 난이도는 시험 전체에서 10-20% 이하로 제한하세요.

### 중(medium) - 다음 중 1개 이상 해당 (기본값)
- 교과서 유형 문제의 응용
- 2-4단계 풀이
- 공식의 변형 적용
- 그래프 해석 및 분석
