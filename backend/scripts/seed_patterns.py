...
3. **학습 처방**
   ...
   - 복습 전략 제안""",
    ...
    # ==========================================
    {
        "name": "난이도 상세 판정 기준",
        "template_type": "analysis_guide",
        "content": """## 난이도 판정 상세 기준 (매우 보수적 적용)

⚠️ **원칙: 확신이 없으면 무조건 한 단계 낮게 판정하세요!**

### 상(high) - 극히 제한적 부여 (매우 엄격)
**반드시 다음 조건을 모두 충족해야 함:**
1. 아래 특징 중 **4개 이상** 명확하게 해당
2. 시험 전체에서 **가장 어려운 1-2문제**에만 해당
3. 배점이 **10점 이상**인 서술형 (예외 없음)
...
- 논리적 증명 요구 (계산만으로 불가능)
- 교과서에 없는 창의적/통찰력 필요
- 일반적으로 학생 정답률 30% 이하 예상

**절대 금지:**
- 교과서 유형 문제는 절대 '상' 불가
- 단순 계산이 주를 이루면 '상' 불가
- 8점 이하 문제는 '상' 불가
- **시험당 상 난이도는 최대 5-10%까지만!**
