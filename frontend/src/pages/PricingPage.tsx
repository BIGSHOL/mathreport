    ...
    return (
        <div className="max-w-5xl mx-auto px-4 py-6">
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-baseline justify-between mb-2">
                                    ...
                                    <div>
                                        {plan.original_price && (
                                            <span className={`text-sm line-through mr-1 ${isCurrentPlan ? 'text-indigo-300' : 'text-gray-400'}`}>
                                ...
                                <div className={`mb-3 py-1.5 px-2 rounded-lg text-center ${isCurrentPlan ? 'bg-indigo-500/30' : 'bg-indigo-50'}`}>
                                    <span className={`text-lg font-bold ${isCurrentPlan ? 'text-white' : 'text-indigo-600'}`}>
                                        {plan.weekly_credits === -1 ? '무제한' : `${plan.weekly_credits}크레딧`}
                                    </span>
                                    <span className={`text-xs ml-1 ${isCurrentPlan ? 'text-indigo-200' : 'text-gray-500'}`}>/주</span>
                                </div>
            ...
            <div>
                ...
                <p className="text-xs text-gray-500 text-center mb-4">분석 요청 시 1크레딧이 소모됩니다. (추후 변경될 수 있음)</p>
                <div className="flex justify-center gap-4 flex-wrap">
                    {packages.map((pkg) => (
                        <div key={pkg.id} className="bg-white border border-gray-200 rounded-lg px-4 py-3 text-center min-w-[120px] hover:border-indigo-300 transition-colors">
                            <div className="text-xl font-bold text-indigo-600">{pkg.credits}크레딧</div>
                            <div className="text-lg font-bold text-gray-900">
                                {pkg.original_price && (
                                    <span className="text-sm line-through text-gray-400 mr-1">₩{pkg.original_price.toLocaleString()}</span>
                                )}
                                ₩{pkg.price.toLocaleString()}
                            </div>
                            <div className="text-xs text-gray-400 mb-2">크레딧당 ₩{pkg.unit_price}</div>
                            {usage?.is_master ? (
